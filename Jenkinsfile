        ...
        stage('Test') {
            steps {
                script {
                    ...
                    // Install specific versions of PHPUnit and its dependencies
                    sh 'docker-compose -f /mnt/myApp/docker-compose.yml --env-file /mnt/myApp/.env run -w /var/www/html app composer require --dev phpunit/phpunit:9.5.10 phpunit/php-code-coverage:9.2.10 phpunit/php-file-iterator:3.0.6 phpunit/php-invoker:3.1.1 phpunit/php-text-template:2.0.2 sebastian/code-unit-reverse-lookup:2.0.2 sebastian/complexity:2.0.2 --ignore-platform-reqs'
